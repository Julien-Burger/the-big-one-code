...
public class Room : MonoBehaviour
{
    ...
    private void Start()
    {
        int villagerGhostSpawn = UnityEngine.Random.Range(0, 2);

        if (bossRoom || !references.GetPlayerController().GetCursed() || villagerGhostSpawn == 1)
        {
    ...
    private void ActivityCheck()
    {
        ...
        if (i == activitiesInCurrentRoom.Length - activityToSkip)
        {
            StartCoroutine(SpawnExitRoom());
        }
    }
    ...
    private IEnumerator SpawnExitRoom()
    {
        ...
        if (SceneManager.GetActiveScene().name == "Tuto") yield break;
    ...
    private void SpawnMerchant()
    {
        ...
        bool achievementUnlock = false;

        if (SteamManager.Initialized)
        {
            SteamUserStats.GetAchievement("ACH_MERCHANT", out achievementUnlock);
        }

        if (spawnChance == 0 && achievementUnlock)
        {
    ...
    public void IncreaseActivityToSkip()
    {
        activityToSkip++;
    }
}
