...
public class EnemyController : MonoBehaviour
{
    ...
    {
        if (stopEnemy && !rebindAnimator)
        {
            if (gameObject.name == "Enemy - Skeleton King")
            {
                enemyDependency.GetEnemyHealth().GetAnimator().SetBool("walk", false);
            }
            else if (gameObject.name == "Enemy - Grim Reaper")
            {
                enemyDependency.GetEnemyHealth().GetAnimator().SetBool("move", false);
            }
            else if (gameObject.name != "Enemy - Bat")
            {
    ...
    private void CheckCollision()
    {
        ...
        if (bodyCollider.IsTouching(references.GetPlayerController().GetBodyCollider()))
        {
            ...
            if (villagerGhost && !references.GetPlayerController().isRolling && !references.GetPlayerHealth().GetHitAlready())
            {
                enemyDependency.GetEnemyHealth().TakeDamage(Mathf.Infinity, true, false);

                references.GetPlayerHealth().TakeDamage(damage, isCritical, true, false, false, true);
            }
