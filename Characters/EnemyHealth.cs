...
public class EnemyHealth : MonoBehaviour
{
    ...
    [SerializeField] AudioSource deathAS;
    ...
    float poisonEffectDuration = 2f;
    float poisonDamagePerSecond = 1.2f;
    ...
    private void Start()
    {
        ...
        {
            baseHealth *= references.GetPlayerStatistics().GetBaseDamage() / 10;
        }
        ...
        if (villagerGhostPrefab)
        {
            villagerGhostPrefab.SetActive(false);
        }
    }
    ...
    private void IsDead()
    {
        if (currentHealth <= 0)
        {
            ...
            if (!GetComponent<VillagerGhost>())
            {
                references.GetCurse().RemoveCurse(Random.Range(4, 7));
            }
            ...
            if (references.GetPassiveObject().BloodTransfusion() && !GetComponent<VillagerGhost>())
            {
    ...
    private IEnumerator DoubleAxe(EnemyHealth enemy)
    {
        ...
        float damage = references.GetFighter().GetDamage(false, false);
    ...
    private void ProcessDamage(float damage, bool isCriticalDamage, bool spawnDamageVFX)
    {
        if (currentArmor > 0f)
        {
            currentArmor -= damage * 0.70f;
            currentHealth -= damage * 0.30f;
    ...
    public void IncreaseHealth(float amount, bool shieldUpgrade)
    {
    ...
    private void DisableGameObject()
    {
        ...
        float villagerGhostSpawn = Random.Range(0f, 1f);

        if (villagerGhostPrefab && references.GetPlayerController().GetCursed() && villagerGhostSpawn >= 0.75f)
        {
            villagerGhostPrefab.SetActive(true);

            villagerGhostPrefab.GetComponent<VillagerGhost>().Incanted(true);
        }
        else if (deathVFX)
        {
            if (spriteRenderer.flipX)
        ...
        if (!references.GetPlayerController().GetCursed() && references.GetRoomPool())
        {
            references.GetRoomPool().GetCurrentRoom().IncreaseActivityToSkip();
        }
        else if (villagerGhostPrefab && villagerGhostSpawn < 0.75f)
        {
            references.GetRoomPool().GetCurrentRoom().IncreaseActivityToSkip();
        }
    }
