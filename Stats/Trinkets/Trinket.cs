...
public class Trinket : MonoBehaviour
{
    ...
    [SerializeField] AudioSource trinketReadyAS;
    ...
    private void Alchemist()
    {
        foreach (EnemyHealth enemy in FindObjectsOfType<EnemyHealth>())
        {
            ...
            {
                float damage = references.GetFighter().GetDamage(false, true);
    ...
    {
        if (references.GetPlayerStatistics().GetArmor() < 15)
        {
            float armor = 15 - references.GetPlayerStatistics().GetArmor();

            references.GetPlayerStatistics().AddArmorPNJBonus(armor);
        }
        
        references.GetPlayerHealth().RegainArmor(15);
        references.GetPlayerHealth().DisplayCurrentArmor();

        textInfoSpawner.SpawnNPCBonus("Armor +15", color);
    }
    ...
    public void Miner()
    {
        ...
        int numberOfGlitchs = UnityEngine.Random.Range(3, 7);
    ...
    {
        if (currentTrinket == TrinketIndex.None) return;
        if (currentChargeValue == chargeValue) return;
        ...
        else
        {
            foreach (TrinketSettings trinket in trinketSettings)
            {
                if (trinketIndex == trinket.trinketIndex)
                {
                    ...
                    StartCoroutine(references.GetTrinketDisplay().SetTrinketSprite(trinketIndex));
                }
            }
