...
public class StatsUpBoard : MonoBehaviour
{
    ...
    private void SetupPropositions()
    {
        ...
        for (int i = 0; i < propositions.Length; i++)
        {
            ...
            int epicChance = UnityEngine.Random.Range(0, 100);
            ...
            while (true)
            {
                ...
                if ((statsPool[statPoolIndex].propositionSettings[propositionIndex].statsUpIndex == StatsUpIndex.DamageForPassive ||
                    ...
                    && (!references.GetPlayerController().GetCursed() || references.GetCurse().GetCurrentCurse() == 0)) continue;
                    ...
                    && (references.GetGameManager().GetCantLoseNextKeySeries() || references.GetGameManager().GetNoobMode())) continue;
                    ...
                    && (references.GetGameManager().GetCantLoseAllKeySeries() || references.GetGameManager().GetNoobMode())) continue;
                    ...
                    && !references.GetSimulationsPlaceHolder().CanRemoveSimulationLevel()) continue;

                else if (statsPool[statPoolIndex].propositionSettings[propositionIndex].statsUpIndex == StatsUpIndex.ReduceWeaponRecharge
                    && references.GetFighter().GetWeaponType() == WeaponType.BrokenSword && !references.GetFighter().GetIsWeaponUpgraded()) continue;

                else if (statsPool[statPoolIndex].propositionSettings[propositionIndex].statsUpIndex == StatsUpIndex.GainRandomPassiveButLostPassive
                    && !references.GetPassiveObject().CanDeleteSlot()) continue;

                else if (statsPool[statPoolIndex].propositionSettings[propositionIndex].statsUpIndex == StatsUpIndex.GainRandomTrinketButLostPassive
                    && !references.GetPassiveObject().CanDeleteSlot()) continue;

                else if (statsPool[statPoolIndex].propositionSettings[propositionIndex].statsUpIndex == StatsUpIndex.AddRoomToSector
                    && !references.GetPlayerController().GetCursed()) continue;

                else if (statsPool[statPoolIndex].propositionSettings[propositionIndex].statsUpIndex == StatsUpIndex.AddPotion
                    && !references.GetPotion().CanAddPotion()) continue;
    ...
    private void GiveBonus()
    {
                ...
                foreach (PropositionSettings item in statPool.propositionSettings)
                {
                    if (item.statsUpIndex == StatsUpIndex.AddRoomToSector)
                    {
                        if (item.isEpicValue)
                        {
                            ...
                            references.GetSector().IncreaseRoomsRemaining((int)item.epicValue);
                        }
                        else
                        {
                            references.GetRoomPool().AddRoomToSector((int)item.normalValue);
                            references.GetSector().IncreaseRoomsRemaining((int)item.normalValue);
                        }
                    }
                }
            }
        ...
        else if (propositions[selectionIndex].name == StatsUpIndex.ReduceWeaponRecharge.ToString())
        {
            foreach (StatsPool statPool in statsPool)
            {
                foreach (PropositionSettings item in statPool.propositionSettings)
                {
                    if (item.statsUpIndex == StatsUpIndex.ReduceWeaponRecharge)
                    {
                        if (item.isEpicValue)
                        {
                            references.GetFighter().ReducePowerRecharge(item.epicValue);
                        }
                        else
                        {
                            references.GetFighter().ReducePowerRecharge(item.normalValue);
                        }
                    }
                }
            }
        ...
        else if (propositions[selectionIndex].name == StatsUpIndex.GainMaxHPForLimitedTimeNormal.ToString())
        {
            foreach (StatsPool statPool in statsPool)
            {
                foreach (PropositionSettings item in statPool.propositionSettings)
                {
                    if (item.statsUpIndex == StatsUpIndex.GainMaxHPForLimitedTimeNormal)
                    {
                        if (item.isEpicValue)
                        {
                            references.GetPlayerStatistics().AddHealthForLimitedTimeNormal(item.epicValue);
                            references.GetPlayerHealth().RefreshCurrentHealth(item.epicValue);
                        }
                        else
                        {
                            references.GetPlayerStatistics().AddHealthForLimitedTimeNormal(item.normalValue);
                            references.GetPlayerHealth().RefreshCurrentHealth(item.normalValue);
                        }
                    }
                }
        ...
        else if (propositions[selectionIndex].name == StatsUpIndex.GainDamageForLimitedTimeNormal.ToString())
        {
            foreach (StatsPool statPool in statsPool)
            {
                foreach (PropositionSettings item in statPool.propositionSettings)
                {
                    if (item.statsUpIndex == StatsUpIndex.GainDamageForLimitedTimeNormal)
                    {
                        if (item.isEpicValue)
                        {
                            references.GetPlayerStatistics().AddDamageForLimitedTimeNormal(item.epicValue);
                        }
                        else
                        {
                            references.GetPlayerStatistics().AddDamageForLimitedTimeNormal(item.normalValue);
                        }
                    }
                }
            }
        ...
        else if (propositions[selectionIndex].name == StatsUpIndex.AddPotion.ToString())
        {
            foreach (StatsPool statPool in statsPool)
            {
                foreach (PropositionSettings item in statPool.propositionSettings)
                {
                    if (item.statsUpIndex == StatsUpIndex.AddPotion)
                    {
                        references.GetPotion().AddPotion();
                    }
                }
            }
                ...
                foreach (PropositionSettings item in statPool.propositionSettings)
                {
                    if (item.statsUpIndex == StatsUpIndex.RemoveSectorRooms)
                    {
                        if (item.isEpicValue)
                        {
                            ...
                            references.GetSector().DecreaseRoomsRemaining((int)item.epicValue, false);
                        }
                        else
                        {
                            references.GetRoomPool().RemoveSectorRooms((int)item.normalValue);
                            references.GetSector().DecreaseRoomsRemaining((int)item.normalValue, false);
                        }
                    }
                }
            }
