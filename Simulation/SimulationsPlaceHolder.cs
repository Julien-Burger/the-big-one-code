...
public class SimulationsPlaceHolder : MonoBehaviour, ISaveable
{
    ...
    public bool CanRemoveSimulationLevel()
    {
        if (references.GetGameManager().GetCurrentSimulationDifficulty() == 0) return false;
        else if (references.GetGameManager().GetCurrentSimulationDifficulty() - numberOfSimulationRemoved == 0) return false;
