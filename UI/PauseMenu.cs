...
public class PauseMenu : MonoBehaviour
{
    ...
    private void OnGoUp()
    {
        ...
        if (selectionIndex < 0 || (selectionIndex < 1 && (SceneManager.GetActiveScene().name == "Tuto" || SceneManager.GetActiveScene().name == "Home")))
        {
    ...
    private void OnGoDown()
    {
        ...
        {
            if (SceneManager.GetActiveScene().name == "Tuto" || SceneManager.GetActiveScene().name == "Home")
            {
                selectionIndex = 1;
            }
            else
    ...
    public void EnablePauseMenu()
    {
        if (SceneManager.GetActiveScene().name == "Tuto" || SceneManager.GetActiveScene().name == "Home")
        {
            ...
            cantUseImg.enabled = true;
        }
        else
        {
            cantUseImg.enabled = false;
        }
    ...
    private IEnumerator SwitchKeySprite(string key)
    {
        ...
        if (references.GetGameManager().GetKeyboardControl())
        {
            if (key == "R")
            {
                ...
                StartCoroutine(references.GetGameManager().ReturnHome(true, false));
            }
            else if (key == "Q")
            {
                Time.timeScale = 1f;
